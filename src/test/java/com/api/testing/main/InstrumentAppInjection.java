...
public class InstrumentAppInjection {
	...
	@Test(priority = 3)
	public static void initiateInstrumentationImageinject() {
		
		String apkInsrument= JsonPayload.initiateInstrumentationImageInjection(token,targetFileName);
	...
	public static void imageInjection()
	{
		String injectImage= JsonPayload.imageInject(token, rid);
		System.out.println("Request response is "+ injectImage);
		
		JsonUtil.printFormattedJson(ApiMethods.executeApiCall("POST", null, null, null,injectImage, ApiEndPoints.IMAGE_INJECT, null));
		System.out.println("Image injection done");
